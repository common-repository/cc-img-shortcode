...
if ( ! class_exists( __NAMESPACE__ . '\Plugin' ) ) {
	class Plugin extends Singleton {
		...
		static public function get( $name = null ) {
			$path = WP_PLUGIN_DIR . '/cc-img-shortcode';
		...
		static public function get_template( $template, $vars = array() ) {
			$template = WP_CONTENT_DIR . '/' . ltrim( self::apply_filters( 'template', $template, $vars ), '/' );
			if ( ! is_file( $template ) ) return '';

			$vars = self::apply_filters( 'vars', $vars, $template );
			if ( is_numeric( $vars['src'] ) or empty( $vars['src'] ) ) return '';
